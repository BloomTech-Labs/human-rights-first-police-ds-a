...
conversation_tree = {
	1:"Hi, do you have more information about location?",
...
def end_conversation(root_id: int, max_step: List):
	test = get_replies('RowenWitt', max_step[2], 'witt_rowen') ## (me, last tweet I sent, person I'm talking to)
...
def advance_conversation(root_id: int) -> str:
	...
	if max_step[6] == 0:
			...
			if test.full_text: # Validation goes here
				...
				if location['status'] == "OK":
					...
					if len(loc_list) != 4:
						...
						# Move conversation to 5, send thank you
					city = loc_list[1]
		...
		## I've sent a tweet, check for response
		# update checks
		# if find response:
			# insert all data to 
			# recursive call to advance conversation?
		#respond_to_tweet(max_step[1], conversation_tree[2])
	# Need to get most recent reply in conversation reply chain
	# Need to flip back and fourth my id : tweeter id until I get it																																										
