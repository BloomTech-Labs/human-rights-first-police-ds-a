...
def advance_conversation(root_id: int, form_link: str = None) -> str:
    ...
    # This is where current quick reply conversation Flow Begins
        ...
        processed_dms = twitter.process_dms(user_id=max_step.in_reply_to_id, tweet_id=max_step.tweet_id,
                                            incident_id=max_step.incident_id, convo_tree_txt=conversation_tree[11])
...
def find_location(query_string: str) -> Dict:
    ...
    query = 'https://maps.googleapis.com/maps/api/place/findplacefromtext/json?input={}&inputtype=textquery&fields=formatted_address,geometry,name,place_id&key={}'.format(
        query_string, MAP_API)
