...
from models import Conversations, DirectMessages, ForceRanks
...
class Database(object):
    ...
    def get_conversation_root(self, root_id: int):
        ...
        with self.Sessionmaker() as session:
            query = select(Conversations).where(
                Conversations.root_tweet_id == root_id)
            conversations_data = session.execute(query)
    ...
    def get_no_location_force_rank(self):
        ...
        with self.Sessionmaker() as session:
            query = select(ForceRanks).filter(
                ForceRanks.like("%Rank 0 - Test"))
            force_ranks_data = session.execute(query)
